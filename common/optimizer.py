...
class SAM(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        raise NotImplementedError("SAM doesn't work like vanilla optimizers. Please call `first_step` and `second_step` in your training loop.")
...
class ESAM(torch.optim.Optimizer):
    ...
    def first_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # SWP: random select with prob beta, scale by 1/beta
    ...
    def second_step(self, zero_grad=False):
        for group in self.param_groups:
            for p in group["params"]:
                if p.grad is None:
                    continue
                p.data = self.state[p]["old_p"].clone()  # Restore original params
        self.base_optimizer.step()
        if zero_grad:
            self.zero_grad()
    ...
    def _grad_norm(self):
        shared_device = self.param_groups[0]["params"][0].device
        norms = [torch.norm((torch.abs(p) if group["adaptive"] else 1.0) * p.grad, p=2).to(shared_device)
