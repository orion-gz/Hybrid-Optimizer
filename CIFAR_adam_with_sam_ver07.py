"""
CIFAR_adam_with_sam_ver07.py

AdamW -> SAM experiment using SharpnessAwareSwitcher.

AdamW phase: warmup 10ep + constant LR (no cosine decay until switch).
SAM phase: restart_lr = initial_lr * lr_restart_factor, rho warmup from rho_min to rho_max.

First run: set sharpness_threshold=999.0 to monitor sharpness without switching,
then pick a threshold based on the logged values.
"""
...
def switch_to_sam(model, optimizer, config):
    """Switch AdamW to SAM(AdamW base) with rho warmup and LR restart.

    Starts at rho_min and linearly increases to rho_max over rho_warmup_epochs.
    This prevents the sharp accuracy drop seen when applying a large rho immediately
    to already-converged weights.
    """
    ...
    new_optimizer = SAM(
        model.parameters(),
        optim.AdamW,
        rho=rho_min,  # start at rho_min; warmup handled by update_rho
        lr=restart_lr,
...
def update_rho(optimizer, epoch, switch_epoch, config):
    """Linearly increase rho from rho_min to rho_max after the switch.

    Clamps to rho_max once the warmup window is complete.
    """
...
def run_experiment(config):
    ...
    if strategy_name == "SAM_Only":
        ...
        # AdamW phase: warmup + constant LR (cosine decay begins only after the switch)
        ...
        constant_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: 1.0)
        scheduler = SequentialLR(optimizer, schedulers=[warmup_scheduler, constant_scheduler],
    ...
    total_training_time = 0
    switched = False
    switch_epoch_num = None  # reference point for rho warmup
    ...
    for epoch in range(config['epochs']):
        ...
        # apply rho warmup every epoch after the switch
        current_rho = None
        ...
        # sharpness-based switch logic
        if "then" in strategy_name and not switched:
            if switcher.step(epoch=epoch, model=model, val_loader=val_loader, criterion=criterion, device=device):
                ...
                if strategy_name in ("AdamW_then_SAM", "AdamW_then_ASAM"):
                    ...
                    optimizer = ESAM(model.parameters(), optim.AdamW, rho=config['rho_min'],
                                     lr=restart_lr, weight_decay=config['weight_decay'], beta=0.5)
