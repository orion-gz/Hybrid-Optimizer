...
def switch_to_sam(model, optimizer, config, epoch):
    """
    AdamW → SAM(AdamW base) 전환 (LR restart 버전).

    LR restart 전략:
        cosine decay로 낮아진 현재 LR을 그대로 쓰면
        SAM perturbation이 의미 있는 탐색을 하기 어려움.
        initial_lr * lr_restart_factor로 재시작해
        SAM이 flat minima를 탐색할 수 있는 여지를 확보.
        시뮬레이션도 동일한 restart_lr로 수행하여 일관성 유지.
    """
...
def run_experiment(config):
    ...
    switcher = DynamicSwitcher_ver04(
