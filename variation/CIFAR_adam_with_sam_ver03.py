"""
CIFAR_adam_with_sam_ver03.py

AdamW -> SAM experiment using DynamicSwitcher_ver03 (slope-only switcher).

Simpler than ver02: switch fires when val_acc improvement rate falls below
slope_threshold over the last slope_window epochs.
AdamW LR is inherited at switch time; no warmup after the switch.
"""
...
from common.switcher import DynamicSwitcher_ver03
from common.train_flow import train_one_epoch, evaluate
from common.config import get_config_ver03, print_config
...
def switch_to_sam(model, optimizer, config, epoch):
    """Switch AdamW to SAM(AdamW base), inheriting the current LR and optimizer state."""
...
def run_experiment(config):
    ...
    switcher = DynamicSwitcher_ver03(
    ...
    for epoch in range(config['epochs']):
        ...
        # slope-based switch logic
        if "then" in strategy_name and not switched:
            if switcher.step(epoch=epoch, val_acc=val_acc):
                ...
                # resume cosine decay from inherited LR, no warmup
...
def plot_results(results):
    ...
    for ax, key_train, key_val, title, ylabel in [
        ...
        for name, history in results.items():
            ax.plot(history[key_train], '--', label=f'{name} Train')
            ax.plot(history[key_val],   '-',  label=f'{name} Val')
            if history.get('switch_epoch'):
                ax.axvline(x=history['switch_epoch'], linestyle=':', alpha=0.7, label=f'{name} Switch')
...
if __name__ == '__main__':
    set_seed(get_config_ver03()['seed'])
