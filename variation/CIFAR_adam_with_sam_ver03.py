...
def switch_to_sam(model, optimizer, config, epoch):
    ...
    - AdamW의 현재 LR을 그대로 인계받아 LR 불연속을 방지
    - base optimizer를 AdamW로 유지해 optimizer state 충격 최소화
    - warmup 없이 남은 epoch 동안 cosine decay 재개
    """
    ...
    new_optimizer = SAM(
        model.parameters(),
        optim.AdamW,                    # SGD 아닌 AdamW 유지
        rho=config['rho'],
        lr=current_lr,                  # AdamW 현재 LR 그대로 인계
        weight_decay=config['weight_decay']
    )

    # AdamW optimizer state(m, v) 이전 - LR 충격 완화에 핵심
    adamw_state = optimizer.state_dict()['state']
    new_opt_state = new_optimizer.base_optimizer.state_dict()
