...
from common.switcher import DynamicSwitcher_ver06
from common.train_flow import train_one_epoch, evaluate
from common.config import get_config_ver06, print_config
...
def switch_to_sam(model, optimizer, config, epoch):
    """
    AdamW → SAM(AdamW base) 전환 (ver06: rho warmup 버전).

    rho warmup 전략:
        전환 직후 rho_min(작은 값)부터 시작해서 rho_warmup_epochs 동안
        rho_max까지 점진적으로 증가. 이렇게 하면:
        - 전환 초기: 작은 perturbation으로 급락 방지 (v4 문제 해결)
        - 점진 증가: SAM 동역학에 적응하며 탐색 강도 증가
        - 최종 도달: rho_max에서 본격적인 flat minima 탐색 (v5 문제 해결)
    """
    ...
    print(f"    └─ SAM rho warmup: {rho_min} → {rho_max}"
          f" over {config['rho_warmup_epochs']} epochs")
    print(f"    └─ base_optimizer: AdamW")

    # rho_min으로 시작 — 전환 충격 최소화
    new_optimizer = SAM(
        model.parameters(),
        optim.AdamW,
        rho=rho_min,
...
def update_rho(optimizer, epoch, switch_epoch, config):
    """
    rho warmup: 전환 후 rho_min → rho_max로 점진 증가.

    Linear schedule:
        progress = (epoch - switch_epoch) / rho_warmup_epochs
...
def run_experiment(config):
    ...
    switcher = DynamicSwitcher_ver06(
    ...
    total_training_time = 0
    switched = False
    switch_epoch_num = None      # [ver06] rho warmup 기준점
    ...
    for epoch in range(config['epochs']):
        ...
        rho_info = f" | rho: {current_rho:.4f}" if current_rho is not None else ""
        print(
            f"Epoch {epoch+1:03d}/{config['epochs']} {phase} | "
            f"LR: {optimizer.param_groups[0]['lr']:.6f}{rho_info} | "
        ...
        if "then" in strategy_name and not switched:
            if switcher.step(
                ...
                switch_epoch_num = epoch + 1    # [ver06] rho warmup 기준점
                ...
                if strategy_name in ("AdamW_then_SAM", "AdamW_then_ASAM"):
                    ...
                    optimizer = ESAM(
                        model.parameters(),
                        optim.AdamW,
                        rho=config['rho_min'],     # [ver06] rho_min으로 시작
                        lr=restart_lr,
                        weight_decay=config['weight_decay'],
                        beta=0.5
                    )
...
if __name__ == '__main__':
    set_seed(get_config_ver06()['seed'])
